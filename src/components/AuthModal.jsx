...
export function AuthModal({ isOpen, onClose, onLogin }) {
  ...
  const handleSubmit = (e) => {
    ...
    // Check for admin/staff accounts with exact credentials
    if (
      formData.email === "admin@techshop.vn" &&
      formData.password === "admin123"
    ) {
      role = "admin";
      isValidLogin = true;
    } else if (
      formData.email === "staff@techshop.vn" &&
      ...
      // For registration, accept any email/password
      isValidLogin = true;
    }
    ...
    if (isValidLogin) {
      const userInfo = {
        ...
        avatar: `https://ui-avatars.com/api/?name=${encodeURIComponent(formData.name || formData.email.split("@")[0])}&background=ef4444&color=fff`,
        role: role,
      };
